...
class WorkoutRemoteDatasourceImpl implements WorkoutRemoteDatasource {
  ...
  Future<List<ExerciseModel>> getExercises(String workoutId) async {
    final response = await apiConsumer.get(EndPoints.getExcersisesByWorkout + workoutId);
    final data = response.data['data'];
    if (response.statusCode == 200) {
  ...
  Future<ExerciseDetailsModel> getExerciseDetails(String exerciseId) async {
    final response = await apiConsumer.get(EndPoints.getExcersisesDetailsById + exerciseId);
    final data = response.data['data'];
    if (response.statusCode == 200) {
